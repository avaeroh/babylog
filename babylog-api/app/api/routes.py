...
from app.domain.models import FeedIn, NappyEventIn, LastOut, StatsOut
...
# ---------- Writes ----------
@router.post("/log/feed", status_code=201)
def log_feed(payload: FeedIn, _=Depends(api_key_auth), session: Session = Depends(get_session)):
...
def log_nappy(payload: NappyEventIn, _=Depends(api_key_auth), session: Session = Depends(get_session)):
    ...
    new_id = repo.add(ts=payload.ts, type=payload.type, description=payload.description)
    return {"id": new_id}

# ---------- Deletes ----------
@router.delete("/last/feed", status_code=status.HTTP_204_NO_CONTENT)
...
# ---------- Last ----------
@router.get("/last/feed", response_model=LastOut)
...
# ---------- Stats ----------
@router.get("/stats/feeds", response_model=StatsOut)
